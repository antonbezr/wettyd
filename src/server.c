...
static void print_help() {
  // clang-format off
  fprintf(stderr, "wettyd is a tool for sharing terminal over the web\n\n"
          "USAGE:\n"
          "    wettyd [options] <command> [<arguments...>]\n\n"
          "VERSION:\n"
          "    %s\n\n"
          "OPTIONS:\n"
...
static int parse_int(char *name, char *str) {
  ...
  if (errno != 0 || endptr == str) {
    fprintf(stderr, "wettyd: invalid value for %s: %s\n", name, str);
    exit(EXIT_FAILURE);
  }
  return (int)val;
}
  ...
  while ((c = getopt_long(start, argv, opt_string, options, NULL)) != -1) {
    switch (c) {
      ...
      case 'p':
        ...
        if (info.port < 0) {
          fprintf(stderr, "wettyd: invalid port: %s\n", optarg);
          return -1;
        }
        break;
      case 'i':
      ...
      case 'c':
        if (strchr(optarg, ':') == NULL) {
          fprintf(stderr, "wettyd: invalid credential, format: username:password\n");
          return -1;
        }
        char b64_text[256];
      ...
      case 's': {
        ...
        if (sig > 0) {
          ...
          fprintf(stderr, "wettyd: invalid signal: %s\n", optarg);
          return -1;
        }
      } break;
      case 'w':
...
#undef sc
      ...
      case 'P': {
        ...
        if (interval < 0) {
          fprintf(stderr, "wettyd: invalid ping interval: %s\n", optarg);
          return -1;
        }
      ...
      case 'f': {
        ...
        if (serv_buf_size < 0) {
          fprintf(stderr, "wettyd: invalid srv-buf-size: %s\n", optarg);
          return -1;
        }
      ...
      case 't':
        ...
        for (; optind < start && *argv[optind] != '-'; optind++) {
          ...
          if (key == NULL) {
            fprintf(stderr, "wettyd: invalid client option: %s, format: key=value\n", optarg);
            return -1;
          }
  ...
  if (server->command == NULL || strlen(server->command) == 0) {
    fprintf(stderr, "wettyd: missing start command\n");
    return -1;
  }
  ...
  char server_hdr[128] = "";
  snprintf(server_hdr, sizeof(server_hdr), "wettyd/%s (libwebsockets/%s)", TTYD_VERSION, LWS_LIBRARY_VERSION);
  ...
  lwsl_notice("wettyd %s (libwebsockets %s)\n", TTYD_VERSION, LWS_LIBRARY_VERSION);
  print_config();
